...
/** Finds the degrees of separation between two Marvel
 * comic book characters, based on co-appearances in a comic.
 */
...
object DegreesOfSeparation {
  ...
  def convertToBFS(line: String): BFSNode = {
    ...
    // Extract this hero ID from the first fields
  ...
  /** Expandas a BFSNode into this node and its children */
  def bfsMap(node: BFSNode): Array[BFSNode] = {
    ...
    // of potencially many BFSNodes to add to our new RDD
    var results: ArrayBuffer[BFSNode] = ArrayBuffer()

    // Gray nodes ara flagged for expansion, and create new
    ...
    if (color == "GRAY") {
      for (connection <- connections) {
        ...
        // Have we sumbled across the character we're looking for?
  ...
  def bfsReduce(data1: BFSData, data2: BFSData): BFSData = {
    ...
    // Se if one is the original node with its connections.
    // If so preserve them.
    if (edges1.length > 0) {
      edges ++= edges1
    }
  ...
  def main(args: Array[String]) {
    ...
    // Create a SparkContext using very core of the local machine
    val sc = new SparkContext("local[*]", "DegreesOfSeparation")

    // Or accumulator, used to signal when we find the target
    ...
    for (iteration <- 1 to 10) {
      ...
      if (hitCounter.isDefined) {
        ...
        if (hitCount > 0) {
          println(s"Hit the target character! From $hitCount different directions(s).")
          return
        }
      }
